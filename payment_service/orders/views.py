...
class Create_Checkout_Session_Order_View(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        promo_codes = [
            {promo.code: promo.id} for promo in stripe.PromotionCode.list()
            ]
        ...
        for promo in PromoCode.objects.all():
            check_list = list(
                filter(
                    lambda x: True if promo.code in x else False, promo_codes
                    )
                )
            if not check_list:
                ...
                stripe.PromotionCode.modify(
                    check_list[0].get(promo.code),
                    metadata={"coupon": promo.coupon.id}
                    )
